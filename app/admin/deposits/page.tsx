...
export default function AdminDepositsPage() {
  ...
  const deleteDeposit = async () => {
    ...
    try {
      await deleteDocument('deposits', dep.id);
      const msg =
        ...
        `<b>Network:</b> ${dep.network.toUpperCase()}`;
      sendTelegramAdminOnly(msg);
      toast({ title: 'Deposit moved to trash' });
      fetchDeposits();
    } catch (err: any) {
      toast({ title: 'Error', description: err.message, variant: 'destructive' });
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Review, approve, or reject user deposit requests.</p>
        </div>
      ...
      <Dialog open={!!deletingDeposit} onOpenChange={() => setDeletingDeposit(null)}>
        <DialogContent>
          ...
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-slate-400">This will move the deposit to trash. You can restore it later from the trash page.</p>
          </div>
          <DialogFooter>
